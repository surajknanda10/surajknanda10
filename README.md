# Hi, I’m Suraj Kumar Nanda 👋

### Senior DevOps Engineer

I build and automate scalable AWS cloud infrastructure using modern DevOps practices. I design CI/CD pipelines, container platforms, and infrastructure-as-code systems that increasingly operate through agentic automation—able to observe system state, make decisions, and act with minimal human intervention.
By combining DevOps fundamentals with AI-assisted workflows, I focus on improving delivery speed, reliability, and operational intelligence. My toolkit spans AWS architecture, Docker, Kubernetes, Terraform, and automation frameworks, with a strong emphasis on autonomous remediation, policy-driven deployments, and intelligent observability to accelerate releases while maintaining resilience and quality.

---

## 🔧 Core Competencies

## 🔧 Core Competencies

<p align="center">
  <img src="https://img.shields.io/badge/AWS-Cloud-orange?logo=amazonaws&logoColor=white" />
  <img src="https://img.shields.io/badge/Azure-Cloud-0078D4?logo=microsoftazure&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-Containerization-2496ED?logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Kubernetes-Orchestration-326CE5?logo=kubernetes&logoColor=white" />
  <img src="https://img.shields.io/badge/Terraform-Infrastructure%20as%20Code-623CE4?logo=terraform&logoColor=white" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/GitHub%20Actions-CI%2FCD-2088FF?logo=githubactions&logoColor=white" />
  <img src="https://img.shields.io/badge/Prometheus-Monitoring-E6522C?logo=prometheus&logoColor=white" />
  <img src="https://img.shields.io/badge/Grafana-Observability-F46800?logo=grafana&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-Scripting-3776AB?logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/Bash-Scripting-4EAA25?logo=gnubash&logoColor=white" />
</p>



---

## 🚀 Experience

### **Senior Software & DevOps Engineer**  
*Bristol Myers Squibb – Hyderabad*  
*Month Year – Present*  
- Architected and maintained scalable infrastructure across AWS and Azure.
- Automated deployment pipelines using Terraform and GitHub Actions.
- Improved reliability and performance of distributed systems with observability tooling.

### **Associate Software Engineer**  
*Innova Solutions – Hyderabad*  
*Month Year – Month Year*  
- Implemented CI/CD workflows with Jenkins & CodePipeline.
- Reduced deployment lead time by X% through automation.
- Collaborated with cross-functional engineering teams to boost delivery velocity.

---

## 🏅 Certifications

<div>
  <a href="https://www.credly.com/badges/e6e50fca-04d7-43e7-86a3-393a973f7069/public_url">
    <img src="https://images.credly.com/images/bd31ef42-d460-493e-8503-39592aaf0458/image.png" width="120"/>
  </a>
  <a href="https://www.credly.com/badges/f6a80af2-dbab-47a5-9695-c51b8021afb7/public_url">
    <img src="https://images.credly.com/size/680x680/images/0e284c3f-5164-4b21-8660-0d84737941bc/image.png" width="120"/>
  </a>
</div>



---

## 📫 Contact

- **LinkedIn:** https://www.linkedin.com/in/suraj-kumar-nanda
- **Email:** surajknanda5@gmail.com
